...
# It finds the project root based on this file's location.
...
def fairness_metrics(y_true, y_pred, protected):
    ...
    # If one group is missing, fairness cannot be calculated
...
# 2. ENCODING FUNCTION
# ============================================================
# Converts all object/string columns into numeric values
...
def run_models(X_train, X_test, y_train, y_test, dataset_name, protected_col):
    ...
    for name, model in models.items():
        ...
        # Compute performance and fairness
...
# ============================================================
# Public benchmark dataset used in the analysis
# ============================================================
...
adult_file = DATA_DIR / "adult.data"
...
# Replace missing markers and remove incomplete rows
...
# ============================================================
# The HR dataset is created by merging three internal Excel files
# ============================================================

hr_core_file = DATA_DIR / "HRDatabase.xlsx"
hr_emp_file = DATA_DIR / "EmployeeInformation.xlsx"
hr_dept_file = DATA_DIR / "DepartmentInformation.xlsx"

hr_core = pd.read_excel(hr_core_file, engine="openpyxl")
hr_emp = pd.read_excel(hr_emp_file, engine="openpyxl")
hr_dept = pd.read_excel(hr_dept_file, engine="openpyxl")

# Standardise column names in all HR files
...
# Merge the three HR files into one dataset
...
# 7. CLEAN HR DATA
# ============================================================
# Remove IDs, leakage-prone fields, and raw datetime columns
# ============================================================

# If gender is missing, create a temporary placeholder column
# This should not normally happen with the real data
...
results_csv = RESULTS_DIR / "baseline_results_with_fairness.csv"
...
for dataset_name in all_results["Dataset"].unique():
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{dataset_name.lower().replace(' ', '_')}_accuracy.png")
    plt.show()
...
for dataset_name in all_results["Dataset"].unique():
    ...
    plt.xticks(rotation=15)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{dataset_name.lower().replace(' ', '_')}_spd.png")
    plt.show()
...
for dataset_name in all_results["Dataset"].unique():
    ...
    plt.xticks(rotation=15)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{dataset_name.lower().replace(' ', '_')}_di.png")
    plt.show()
...
for dataset_name in all_results["Dataset"].unique():
    ...
    plt.xticks(rotation=15)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{dataset_name.lower().replace(' ', '_')}_eod.png")
    plt.show()
